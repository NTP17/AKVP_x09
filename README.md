...
Outputting control signals and RAM stuff is slightly more complicated. The [parallel-to-serial converter](debugger/PISO.vhd) utilizes DE10-Standard's internal 50 MHz clock to continuously generate three lines of serial data: data input, shift clock, and output clock, which will then fed into respective inputs of [74HC595](https://assets.nexperia.com/documents/data-sheet/74HC_HCT595.pdf), which is chained together with more 74HC595's to output the serial data back to parallel, forming a 3-to-28 converter for control signals, and 3-to-16 for RAM.
